...
- Integrate numeric-prior replay during the streaming source run itself, then test whether the source learns seen and heldout prompts without post-hoc replay; only after that consider trusted handoff.
...
Next allowed test: Integrate numeric-prior replay during the streaming source run itself, then test whether the source learns seen and heldout prompts without post-hoc replay; only after that consider trusted handoff.
Source: `aiAgentWorkHistory/phase7/2026-05-31-amortized-prior-heldout-diagnostic.md`
```
