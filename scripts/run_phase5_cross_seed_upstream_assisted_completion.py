...
RUN_ROOT = (
    REPO_ROOT
    / "runs"
    / "2026-05-08_phase5_cross_seed_upstream_assisted_completion"
)
...
SOURCE_STAGE1_CHECKPOINT = (
    ...
    "checkpoint_snapshots/step_00025_weights.pt"
)
SOURCE_STAGE1_RUN = SOURCE_STAGE1_CHECKPOINT.parents[1]
FROZEN_BASELINE_RUN = (
    PHASE4_BOUNDARY_ROOT
    / "stage2/seed5/step25/2026-05-08_072232_382502_model-c-op0-19-"
    "adaptive_interface-inlr0.0003-uplr0.0003-answer_decoder-sum_left_operand/"
    "model-c-2digit-seed5"
)
FROZEN_BASELINE_CHECKPOINT = FROZEN_BASELINE_RUN / "final_weights.pt"
RETAINED_STEP30_CHECKPOINT = (
    PHASE4_MIN_ROOT
    / "stage2_lower/seed5/step30/2026-05-07_173039_095164_model-c-op0-19-"
    "adaptive_interface-inlr0.0003-uplr0.0003-answer_decoder-sum_left_operand/"
    "model-c-2digit-seed5/final_weights.pt"
)
...
CONDITIONS = {
    "primary": {
        ...
        "description": "answer-only continuation from seed 5 step 25 with upstream trainable",
    },
    "lower_lr": {
...
def source_stage1_metrics() -> dict[str, float | int]:
    ...
    raise ValueError("missing source Stage 1 step 25 metrics")
...
def baseline_diagnostics() -> dict[str, Any]:
    canonical = (
        FROZEN_BASELINE_RUN
        / "seed5_step25_canonical_causal_diagnostics/diagnostic_summary.json"
    )
    private = (
        FROZEN_BASELINE_RUN
        / "seed5_step25_private_protocol_diagnostics/private_protocol_summary.json"
    )
    full_enum = find_full_enum_summary(FROZEN_BASELINE_RUN / "seed5_step25_full_enum_action_loss")
    payload = {
...
def baseline_summary() -> dict[str, Any]:
    return {
        **metric_snapshot_for_run(FROZEN_BASELINE_RUN),
        "label": "existing_frozen_upstream_step25_failure",
...
def retained_reference_summary() -> dict[str, Any]:
    run_dir = RETAINED_STEP30_CHECKPOINT.parent
    return {
        **metric_snapshot_for_run(run_dir),
        "label": "retained_seed5_step30_reference",
...
def starting_point_summary() -> dict[str, Any]:
    return {
        ...
        "retained_step30_reference": retained_reference_summary(),
    }
...
def run_conditions(condition_keys: list[str], jobs: int) -> None:
    ...
    if "starting_point" not in manifest:
        ...
        if existing_summary_path.exists():
            manifest["starting_point"] = load_json(existing_summary_path)["starting_point"]
        else:
...
def write_summary() -> None:
    ...
    summary = {
        "claim": (
            ...
            "seed 5 step 25 partially taught calculator protocol recover after "
        ...
        "interpretation": [
            (
                "The primary upstream-open continuation replicated upstream-assisted "
                "completion on a second failed handoff: its step 950 checkpoint reached "
                "canonical/private operand, pair, and calculator-result accuracy 1.0 "
                "with zero full-enum learned-minus-true/best gaps."
            ),
            (
                "This is checkpoint-selected completion rather than all-snapshot "
                "stability. The primary final checkpoint slipped mildly, and the "
                "anchor repeat reached an exact step 800 checkpoint but drifted more "
                "strongly by step 900/final."
            ),
            (
                "The result is not pure discovery from scratch. It starts from a "
                "partially taught seed 5 step 25 protocol, keeps direct teacher losses "
                "removed in continuation, and allows upstream plus input projection "
                "parameters to move while the semantic decoder stays frozen."
            ),
        ],
    }
...
def write_summary_md(summary: dict[str, Any]) -> None:
    ...
    lines = [
        "# Phase 5 Cross-Seed Upstream-Assisted Completion",
        "",
        ...
        f"- Retained step 30 reference: `{starting['retained_step30_reference']['run_dir']}`",
        "",
        "## Fast gates",
        "",
    ...
    retained_reference = starting["retained_step30_reference"]
    ...
    diagnostic_rows = [
        ("existing_frozen_upstream_step25_failure", baseline.get("diagnostics", {}))
