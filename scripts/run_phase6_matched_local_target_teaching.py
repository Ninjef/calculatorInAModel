...
from scripts.diagnose_calculator_protocol import load_checkpoint, pick_device  # noqa: E402
...
RUN_ROOT = REPO_ROOT / "runs" / "2026-05-10_phase6_matched_local_target_teaching"
...
def cmd_compare_local_target_to_aux(args: argparse.Namespace) -> None:
    log_command([sys.executable, "scripts/run_phase6_matched_local_target_teaching.py", *sys.argv[1:]])
    device = pick_device()
...
def cmd_run_stage1(args: argparse.Namespace) -> None:
    if not parity_gate_passed() and not args.force:
        raise SystemExit(
            "parity gate has not passed; run compare-local-target-to-aux first "
            "or pass --force for development only"
        )
    label = args.label
    if label is None:
        frozen = "frozen" if args.freeze_upstream_encoder else "upstream_open"
        label = f"{frozen}_inlr{args.input_proj_lr:g}_local{args.local_target_loss_weight:g}"
    run_command(
...
def cmd_run_retention(args: argparse.Namespace) -> None:
    ...
    if args.checkpoint is not None:
        starts = [(args.label or "manual", args.checkpoint)]
    else:
        ...
        starts = [(f"{first['label']}_first_gate_step{first['step']:05d}", first["checkpoint"])]
        if best["checkpoint"] != first["checkpoint"]:
            starts.append((f"{best['label']}_best_gate_step{best['step']:05d}", best["checkpoint"]))
    for label, checkpoint in starts:
        run_command(
            phase6_train_command(
                checkpoint=checkpoint,
                run_root=RUN_ROOT / "stage2" / "frozen_upstream_retention" / label,
...
def selected_checkpoints() -> dict[str, Path]:
    checkpoints: dict[str, Path] = {"stage0b": STAGE0B_CHECKPOINT}
    for run_dir in all_model_runs(RUN_ROOT / "stage1"):
        label = run_dir.parents[1].name
...
def cmd_summarize(args: argparse.Namespace) -> None:
    ...
    for stage in ["stage1", "stage2"]:
        for run_dir in all_model_runs(RUN_ROOT / stage):
            ...
            source = STAGE0B_CHECKPOINT if stage == "stage1" else Path(load_json(run_dir / "metrics.json")["semantic_decoder_checkpoint"])
    ...
    lines = [
        "# Phase 6 Matched Local-Target Teaching Summary",
        "",
        f"Run root: `{RUN_ROOT}`",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Phase 6 matched local-target teaching and retention runner."
    )
...
def main() -> None:
    args = parse_args()
    if args.command == "compare-local-target-to-aux":
