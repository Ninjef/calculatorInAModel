...
RUN_ROOT = REPO_ROOT / "runs" / "2026-05-11_phase6_strict_local_target_decay_boundary"
PRIOR_STRICT_RUN_ROOT = REPO_ROOT / "runs" / "2026-05-11_phase6_strict_random_upstream_local_target"
RUNNER_SCRIPT = "scripts/run_phase6_strict_local_target_decay_boundary.py"
...
def prior_strict_stage1_root() -> Path:
    return (
        PRIOR_STRICT_RUN_ROOT
        / "stage1"
        / "semantic_decoder_only_branch_a_frozen_upstream_inlr0.03"
    )
...
def phase6_train_command(
    *,
    ...
    local_target_decay_steps: int,
    local_target_floor: float,
    input_proj_lr: float,
    upstream_lr: float,
    steps: int,
    snapshot_every: int,
    ...
    command = [
        ...
        str(local_target_decay_steps),
        "--adaptive-interface-loss-floor",
        str(local_target_floor),
...
def cmd_run_stage1(args: argparse.Namespace) -> None:
    ...
    if label is None:
        ...
        label = (
            ...
            f"_decay{args.local_target_decay_steps}_floor{args.local_target_floor:g}"
        )
    run_command(
        phase6_train_command(
            checkpoint=args.checkpoint or STAGE0B_CHECKPOINT,
            ...
            local_target_decay_steps=args.local_target_decay_steps,
            local_target_floor=args.local_target_floor,
            input_proj_lr=args.input_proj_lr,
            upstream_lr=args.upstream_lr,
            steps=args.steps,
...
def cmd_run_decay_ladder(args: argparse.Namespace) -> None:
    if (not oracle_wiring_gate_passed() or not parity_gate_passed()) and not args.force:
        raise SystemExit(
            "oracle wiring and parity gates have not both passed under this runner; run "
            "oracle-wiring-gate and compare-local-target-to-aux first "
            "or pass --force for development only"
        )
    for decay_steps in args.decay_steps:
        frozen = "frozen" if args.freeze_upstream_encoder else "upstream_open"
        label = (
            f"{args.semantic_decoder_checkpoint_load_scope}_{frozen}"
            f"_inlr{args.input_proj_lr:g}_local{args.local_target_loss_weight:g}"
            f"_decay{decay_steps}_floor{args.local_target_floor:g}"
        )
        run_command(
            phase6_train_command(
                checkpoint=args.checkpoint or STAGE0B_CHECKPOINT,
                ...
                answer_loss_weight=args.answer_loss_weight,
                local_target_loss_weight=args.local_target_loss_weight,
                local_target_decay_steps=decay_steps,
                local_target_floor=args.local_target_floor,
                input_proj_lr=args.input_proj_lr,
                upstream_lr=args.upstream_lr,
                steps=args.steps,
...
def cmd_run_minimum_handoff(args: argparse.Namespace) -> None:
    starts: list[tuple[str, Path, bool]] = []
    if args.checkpoint is not None:
        starts.append((args.label or "manual", args.checkpoint, args.freeze_upstream_encoder))
    else:
        prior_runs = all_model_runs(prior_strict_stage1_root())
        if not prior_runs:
            raise FileNotFoundError(f"no prior strict Stage 1 run found under {prior_strict_stage1_root()}")
        prior_run = prior_runs[-1]
        for step in args.steps_from:
            checkpoint = prior_run / "checkpoint_snapshots" / f"step_{step:05d}_weights.pt"
            if not checkpoint.exists():
                print(f"Skipping missing prior checkpoint: {checkpoint}")
                continue
            starts.append((f"prior_strict_stage1_step{step:05d}", checkpoint, args.freeze_upstream_encoder))
    if not starts:
        print("No minimum-handoff checkpoints found; nothing to run.")
        return
    for label, checkpoint, freeze_upstream in starts:
        retention_kind = "frozen_upstream_minimum_handoff" if freeze_upstream else "upstream_open_minimum_handoff"
        run_command(
            phase6_train_command(
                checkpoint=checkpoint,
                run_root=RUN_ROOT / "stage2" / retention_kind / label,
...
def compact_run(run_dir: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "local_target_loss_weight": metrics.get("local_target_loss_weight"),
        "local_target_loss_decay_steps": metrics.get("local_target_loss_decay_steps"),
        "local_target_loss_floor": metrics.get("local_target_loss_floor"),
...
def cmd_summarize(args: argparse.Namespace) -> None:
    ...
    lines = [
        "# Phase 6 Strict Local-Target Decay Boundary Summary",
        "",
        f"Run root: `{RUN_ROOT}`",
    ...
    for label, row in runs.items():
        ...
        lines.append(
            ...
            f"answer={row['answer_loss_weight']}, local_init={row['local_target_loss_weight']}, "
            f"decay={row['local_target_loss_decay_steps']}, floor={row['local_target_loss_floor']}, "
            f"local_final={row['final_local_target_loss_weight']}, "
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Phase 6 strict local-target decay and minimum handoff boundary runner."
    )
    ...
    stage1.add_argument("--local-target-decay-steps", type=int, default=0)
    stage1.add_argument("--local-target-floor", type=float, default=0.0)
    ...
    decay = subparsers.add_parser("run-decay-ladder")
    decay.add_argument("--checkpoint", type=Path, default=None)
    decay.add_argument(
        "--semantic-decoder-checkpoint-load-scope",
        choices=["full_model", "semantic_decoder_only"],
        default=DEFAULT_LOAD_SCOPE,
    )
    decay.add_argument("--decay-steps", type=int, nargs="+", default=[50, 75, 100, 150])
    decay.add_argument("--answer-loss-weight", type=float, default=1.0)
    decay.add_argument("--local-target-loss-weight", type=float, default=1.0)
    decay.add_argument("--local-target-floor", type=float, default=0.0)
    decay.add_argument("--input-proj-lr", type=float, default=0.03)
    decay.add_argument("--upstream-lr", type=float, default=0.003)
    decay.add_argument("--steps", type=int, default=300)
    decay.add_argument("--snapshot-every", type=int, default=25)
    decay.add_argument("--checkpoint-every", type=int, default=25)
    decay.add_argument("--seed", type=int, default=0)
    decay.add_argument("--target-mode", choices=["hard_best_pair", "soft_pair"], default="hard_best_pair")
    decay.add_argument("--freeze-upstream-encoder", action=argparse.BooleanOptionalAction, default=True)
    decay.add_argument("--force", action="store_true")
    ...
    handoff = subparsers.add_parser("run-minimum-handoff")
    handoff.add_argument("--checkpoint", type=Path, default=None)
    handoff.add_argument("--label", type=str, default=None)
    handoff.add_argument("--steps-from", type=int, nargs="+", default=[25, 50, 75])
    handoff.add_argument("--answer-loss-weight", type=float, default=1.0)
    handoff.add_argument("--input-proj-lr", type=float, default=0.0003)
    handoff.add_argument("--upstream-lr", type=float, default=0.00003)
    handoff.add_argument("--continuation-steps", type=int, default=1000)
    handoff.add_argument("--snapshot-every", type=int, default=50)
    handoff.add_argument("--checkpoint-every", type=int, default=50)
    handoff.add_argument("--seed", type=int, default=0)
    handoff.add_argument("--freeze-upstream-encoder", action=argparse.BooleanOptionalAction, default=True)
...
def main() -> None:
    ...
    if args.command == "oracle-wiring-gate":
        ...
        cmd_run_stage1(args)
    elif args.command == "run-decay-ladder":
        cmd_run_decay_ladder(args)
    elif args.command == "run-retention":
        cmd_run_retention(args)
    elif args.command == "run-minimum-handoff":
        cmd_run_minimum_handoff(args)
