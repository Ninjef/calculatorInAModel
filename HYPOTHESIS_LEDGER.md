...
| Online hard result-boundary memory + numeric prior | Active benchmark, constrained | Full-memory numeric-prior replay fixes the heldout prompt source gate and clears trusted handoff. Fitting every other step preserves quality with half the prior updates, while every-10 underfits; continue with convergence-gated or coreset prior fitting rather than cadence ladders. |
