...
| Online hard result-boundary memory + numeric prior | Active benchmark, constrained | Full-memory numeric-prior replay fixes the heldout prompt source gate and clears trusted handoff. Every-2 plus sustained train-memory convergence cuts prior updates to `1889`; first-hit convergence, every-10 cadence, and random half-memory fits underfit. Target-stratified half-memory is the first structured coreset positive and clears source/handoff, but still uses `2501` prior updates. Validation-heldout stopping reduced updates only slightly and missed heldout. Eval-only validation stopping clears op19 source/handoff with `1613-1784` prior updates but can raise forced-result evals. Its op29 constant-batch stress missed heldout (`0.8444`) even though post-hoc full-memory/richer-prior diagnostics recovered `0.9000-0.9278`, so next work must change prior capacity/fit dynamics rather than rerun batch160 or validation ladders. |
