...
| Online hard result-boundary memory + numeric prior | Active benchmark, constrained | Full-memory numeric-prior replay fixes the heldout prompt source gate and clears trusted handoff. Every-2 plus sustained train-memory convergence cuts prior updates to `1889`; first-hit convergence, every-10 cadence, and random half-memory fits underfit. Target-stratified half-memory is the first structured coreset positive and clears source/handoff, but still uses `2501` prior updates. Validation-heldout stopping reduced updates only slightly and missed heldout. Eval-only validation stopping clears op19 source/handoff with `1613-1784` prior updates but can raise forced-result evals. Its op29 constant-batch h64/h128 stresses missed heldout (`0.8444`/`0.8611`) despite post-hoc h128 full-memory reaching `0.9278`; post-memory-fill full refresh then cleared op29 source heldout (`0.9167`) and trusted handoff (`1.0000`) but uses `2500` full-memory refresh updates. Simple validation-stop during full refresh cut updates to `1140` but missed heldout (`0.8167`); adding a train-memory guard preserved a fresh source/handoff gate with `2570` updates. Quality-gated cap/freeze is the op29 cost lead and has fresh-seed handoff replication, but many-calculator accounting shows per-calculator prompt memory/prior fitting still scales linearly. Route-heldout diagnostics show a structured numeric prior can generalize targets from three routed calculators to the unscored fourth (`0.933-0.979`), while embedding memorization fails (`0.000`). Next work should train with shared route/global target-prior learning or replace answer-derived scoring, not rerun cap/seed/fit ladders. |
