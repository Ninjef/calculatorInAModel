...
| Online hard result-boundary memory + numeric prior | Active benchmark, constrained | Full-memory numeric-prior replay fixes the heldout prompt source gate and clears trusted handoff. Every-2 plus sustained train-memory convergence cuts prior updates to `1889`; first-hit convergence, every-10 cadence, and random half-memory fits underfit. Target-stratified half-memory is the first structured coreset positive and clears source/handoff, but still uses `2501` prior updates. Validation-heldout stopping reduced updates only slightly and missed heldout. Eval-only validation stopping clears source/handoff with `1613-1784` prior updates but can raise forced-result evals by delaying memory fill, so stress that mechanism and investigate total source-acquisition cost; do not run random batch or validation-heldout threshold ladders. |
