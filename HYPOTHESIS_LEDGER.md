...
| Online hard result-boundary memory + numeric prior | Active benchmark, constrained | Full-memory numeric-prior replay fixes the heldout prompt source gate and clears trusted handoff, but full-memory prior fitting is not yet scalable; continue by reducing prior-fit cost while preserving heldout/handoff behavior. |
